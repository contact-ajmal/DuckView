...
export class SavedQueryService {
  ...
  async create(p: Principal, workspaceId: string, input: { name: string; folder?: string; description?: string | null; sql_text: string; tags?: string[] }): Promise<SavedQuery> {
    ...
    await this.revisions?.record(p, workspaceId, 'query', q.id);
    return q;
  }
  ...
  async update(p: Principal, workspaceId: string, id: string, patch: { name?: string; folder?: string; description?: string | null; sql_text?: string; tags?: string[] }): Promise<SavedQuery> {
    ...
    await this.revisions?.record(p, workspaceId, 'query', id);
    return { ...existing, ...set };
  }
...
export class DashboardService {
  /** Version history (set by the context). */
  revisions: { record(actor: string | null | { userId: string; actorType: 'USER' | 'AGENT' | 'SYSTEM' }, workspaceId: string, type: 'notebook' | 'dashboard' | 'query' | 'semantic' | 'dbt', id: string, opts?: { message?: string | null }): Promise<unknown>; forget(type: 'notebook' | 'dashboard' | 'query' | 'semantic' | 'dbt', id: string): Promise<void> } | null = null;
  private async rec(p: Pick<Principal, 'userId' | 'actorType'>, id: string): Promise<void> {
    ...
    if (d) await this.revisions.record({ userId: p.userId, actorType: p.actorType }, d.workspace_id, 'dashboard', id);
  }
  constructor(private readonly store: MetadataStore, private readonly workspaces: WorkspaceService) {}
  private get db() {
    return this.store.db;
