  ...
  return [
    ...
    define({
      ...
      async handler(env, { workspace_id }) {
        ...
        const lakes = (await env.ctx.lakehouse.list(env.principal.userId)).filter((l) => l.provider !== 'DATABRICKS');
        ...
        const conns = [...dbs.map((c) => `- database \`${c.id}\` ${c.name} (${c.engine})`), ...clouds.map((c) => `- cloud \`${c.id}\` ${c.name} (${c.provider}${c.bucket ? `, bucket ${c.bucket}` : ''}) — files or a Delta table`), ...lakes.map((l) => `- Iceberg catalog \`${l.id}\` ${l.name} (${l.provider})`)];
    ...
    define({
      ...
      description: 'Saves a reverse sync: a read-only SELECT whose rows are sent out of the workspace. destination: {kind:"database", connection_id, schema?, table} (a connection with read-only off; the table is created if missing) · {kind:"file", format: parquet|csv|json, path (relative to the data directory, or a key with cloud_connection_id and bucket)} · {kind:"http", url, batch_size?, payload: object|array|ndjson} (headers such as Authorization go in headers) · {kind:"iceberg", connection_id (a lakehouse connection), namespace, table, storage_connection_id?} (an Iceberg table, created if missing; every mode) · {kind:"delta", path, cloud_connection_id?, bucket?} (a Delta Lake table; replace or append). mode: replace (default), append, upsert or mirror (upsert + delete rows that disappeared) — upsert and mirror need key_columns and send only rows new or changed since the last run. Nothing is sent until run_reverse_sync; a person sets the schedule.',
      inputSchema: {
        name: z.string().min(1).max(120),
        sql: z.string().max(100_000),
