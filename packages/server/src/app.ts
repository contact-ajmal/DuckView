...
import { hostedAgentRoutes } from './routes/hosted-agents.js';
import { a2aRoutes, a2aPublicRoutes } from './routes/a2a.js';
...
import { endpointAdminRoutes, endpointPublicRoutes } from './routes/endpoints.js';
...
export async function buildApp(ctx: AppContext): Promise<{ app: FastifyInstance; mcpSessions: McpSessionRegistry; appsServer: FastifyInstance | null }> {
  ...
  await app.register(async (r) => notebookRoutes(r, ctx));
  await app.register(async (r) => insightRoutes(r, ctx));
  await app.register(async (r) => hostedAgentRoutes(r, ctx));
  await app.register(async (r) => a2aRoutes(r, ctx));
  await app.register(async (r) => a2aPublicRoutes(r, ctx));
  await app.register(async (r) => streamRoutes(r, ctx));
  ...
  await app.register(async (r) => usageRoutes(r, ctx));
  await app.register(async (r) => templateRoutes(r, ctx));
  ...
  await app.register(async (r) => endpointAdminRoutes(r, ctx));
