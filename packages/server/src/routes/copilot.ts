  ...
  app.post('/api/copilot/chat', async (req, reply) => {
    const body = z
      .object({
        ...
        notebook_id: z.string().max(64).nullable().optional(),
        page: z.object({ kind: z.string().max(20), id: z.string().max(500).nullable().optional(), label: z.string().max(300) }).nullable().optional(),
