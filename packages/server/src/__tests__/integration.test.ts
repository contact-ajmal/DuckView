...
describe('MCP server (in-memory transport)', () => {
  ...
  it('lists tools, resources, prompts', async () => {
    ...
    expect(tools).toEqual(['browse_storage', 'create_dashboard_widget', 'create_mosaic_dashboard', 'execute_query', 'explain_query', 'inspect_schema', 'lakehouse_query', 'list_accessible_data', 'list_dashboards', 'profile_dataset', 'save_dataset']);
    ...
    expect((await client.listPrompts()).prompts.map((p) => p.name).sort()).toEqual(['build_mosaic_dashboard', 'data_quality_audit', 'sql_optimization']);
  });
...
describe('HTTP API + network MCP', () => {
  ...
  it('login, workspaces, query, tokens', async () => {
    ...
    expect((info.json.tools as string[]).length).toBe(11);
  });
