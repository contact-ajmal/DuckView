...
describe('HTTP API + network MCP', () => {
  ...
  it('login, workspaces, query, tokens', async () => {
    ...
    expect((info.json.tools as string[]).length).toBe(78);
  });
  it('MCP over Streamable HTTP with bearer token', async () => {
    ...
    expect(tools.tools.length).toBe(78);
