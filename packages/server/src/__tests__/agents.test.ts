...
describe('registered agents', () => {
  ...
  it('publishes an OpenAPI 3.0 document Bedrock action groups can consume', async () => {
    ...
    expect(names.sort()).toEqual(['browse_storage', 'create_dashboard_widget', 'execute_query', 'explain_query', 'inspect_schema', 'lakehouse_query', 'list_accessible_data', 'list_dashboards', 'profile_dataset', 'save_dataset']);
