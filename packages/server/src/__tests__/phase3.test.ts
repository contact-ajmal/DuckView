...
describe('MCP: browse_storage, inspect_schema, list_dashboards, create_dashboard_widget', () => {
  ...
  it('exposes twenty-five tools', async () => {
    expect((await client.listTools()).tools.map((t) => t.name).sort()).toEqual(['browse_connector', 'browse_storage', 'connector_query', 'create_app', 'create_dashboard_widget', 'create_data_sync', 'create_mosaic_dashboard', 'execute_query', 'explain_query', 'get_app_logs', 'inspect_schema', 'lakehouse_query', 'list_accessible_data', 'list_apps', 'list_dashboards', 'list_data_sources', 'preview_app', 'profile_dataset', 'publish_app', 'run_app', 'run_data_sync', 'save_dataset', 'stop_app', 'update_app', 'update_data_sync']);
  });
