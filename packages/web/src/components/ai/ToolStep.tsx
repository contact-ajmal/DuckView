  ...
  return (
    <li className={cn('min-w-0', className)} data-tool={step.tool} data-status={step.status}>
      <button className="group flex w-full min-w-0 items-center gap-2 py-0.5 text-left" onClick={() => setOpen((o) => !o)} aria-expanded={open}>
        <ChevronRight className={cn('h-3 w-3 shrink-0 text-zinc-500 transition-transform duration-[var(--dur-fast)]', open && 'rotate-90')} />
        ...
        {step.durationMs != null && <span className="shrink-0 text-2xs tabular-nums text-zinc-500">{step.durationMs < 1000 ? `${Math.round(step.durationMs)} ms` : `${(step.durationMs / 1000).toFixed(1)} s`}</span>}
      </button>
      ...
      {open && (
        <div className="ml-[2.1rem] mt-1 space-y-1 rounded-md border border-zinc-800 bg-zinc-900/60 p-2 font-mono text-2xs text-zinc-400">
          <div><span className="text-zinc-500">tool</span> {step.tool}</div>
