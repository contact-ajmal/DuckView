...
export type StreamConfig =
  ...
  | { kind: 'http' };
export interface Stream { id: string; workspace_id: string; name: string; kind: 'kafka' | 'kinesis' | 'http'; config: StreamConfig; format: 'json' | 'text'; target_schema: string; target_table: string; include_metadata: boolean; batch_rows: number; batch_seconds: number; enabled: boolean; status: 'stopped' | 'starting' | 'running' | 'error'; stats: { rows_total: number; batches: number; last_batch_rows: number; last_batch_at: string | null; last_error: string | null; last_error_at: string | null }; checkpoints: Record<string, string>; secret_set: boolean; push_url: string | null; created_at: string; updated_at: string }
