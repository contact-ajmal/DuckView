...
type Tab = 'dbt' | 'metrics' | 'quality' | 'prepare';

/** Data › Prepare (recipes), Data › Models (dbt projects), Data › Metrics (the semantic layer) and Data › Quality; the section tabs are in the shell. */
export function TransformPage() {
  const ws = useWorkspace();
  const parse = (): Tab => {
    const t = /^#\/transform\/([a-z]+)/.exec(location.hash)?.[1];
    return t === 'metrics' || t === 'quality' || t === 'prepare' ? t : 'dbt';
  };
  ...
  return (
    <div className="@container h-full min-h-0 overflow-auto">
      <div className="mx-auto max-w-[1400px] px-6 py-5">
        ...
        {ws.activeId && tab === 'prepare' && <PreparePage key={ws.activeId} workspaceId={ws.activeId} />}
