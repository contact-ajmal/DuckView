...
type Tab = 'dbt';
const TABS: { id: Tab; label: string }[] = [{ id: 'dbt', label: 'dbt projects' }];

/** #/transform — turning raw tables into modelled ones: dbt projects. */
    ...
    <div className="h-full min-h-0 overflow-auto">
      <div className="mx-auto max-w-7xl space-y-4 p-5 pb-16">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">Raw tables into tested, documented models in <b className="text-zinc-300">{ws.workspaces.find((w) => w.id === ws.activeId)?.name ?? 'the active workspace'}</b>.</p>
        </div>
