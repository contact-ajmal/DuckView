    ...
    <div className="flex h-full min-h-0">
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-zinc-800 px-4 py-2">
          <span className="truncate font-mono text-body text-zinc-100" title={target}>{target}</span>
          {result && (
          ...
          {result?.tables && result.tables.length > 0 && (
            <div className="ml-auto flex gap-1 overflow-x-auto">
              {result.tables.map((t, i) => (
                <button key={`${t.schema}.${t.name}`} onClick={() => setActiveTable(i)} className={cn('flex shrink-0 items-center gap-1 rounded-md px-2 py-0.5 font-mono text-2xs', i === activeTable ? 'bg-zinc-800 text-zinc-50' : 'text-zinc-400 hover:text-zinc-200')}>
      ...
      {result && (
        <aside className="flex w-80 shrink-0 flex-col gap-2 border-l border-zinc-800 p-3">
          <div className="text-2xs font-semibold text-zinc-500">{remoteConnectionId ? 'Query on the SQL warehouse' : 'Query this file'}</div>
          <pre className="min-h-0 flex-1 overflow-auto rounded-md border border-zinc-800 bg-zinc-900 p-2 font-mono text-2xs text-zinc-300">{suggested}</pre>
