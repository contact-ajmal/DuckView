  ...
  return (
    <Modal open={open} onClose={onClose} title="Add folder to workspace" width="max-w-xl">
      <div className="space-y-3">
        ...
        {state?.mode === 'sandboxed' && <p className="text-[11px] text-amber-300">This server runs in sandboxed mode: only folders inside the data directory can be added.</p>}
        <div className="h-72 overflow-auto rounded-md border border-zinc-800 bg-zinc-950">
          ...
          {state?.entries.map((e) => (
            <div key={e.path} className="group flex items-center gap-2 border-b border-zinc-800/60 px-3 py-1.5 text-xs hover:bg-zinc-800/60">
              ...
              {e.data_files > 0 && <span className="font-mono text-[10px] text-zinc-500">{e.data_files} data file{e.data_files === 1 ? '' : 's'}</span>}
              <button className={cn('rounded px-1.5 py-0.5 text-[10px] text-accent-300 opacity-0 hover:bg-accent-600/20 group-hover:opacity-100')} onClick={() => void pick(e.path)}>add</button>
            </div>
          ))}
        </div>
        {error && <div className="rounded-md border border-red-900 bg-red-950/50 px-3 py-2 text-xs text-red-200">{error}</div>}
        <div className="flex items-center justify-between">
          <span className="truncate font-mono text-[11px] text-zinc-500" title={state?.path}>{state?.path}</span>
