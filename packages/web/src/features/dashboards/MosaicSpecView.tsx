...
export interface SpecRenderStatus {
  ...
  warnings: string[];
  /** Query errors individual charts/inputs hit after rendering (the rest of the dashboard stays up). */
  problems: string[];
...
export function MosaicSpecView({ workspaceId, spec, onStatus, className, nonce = 0 }: { workspaceId: string; spec: Spec | null; onStatus?: (s: SpecRenderStatus) => void; className?: string; nonce?: number }) {
  ...
  useEffect(() => {
    ...
    if (!spec || Object.keys(spec).length === 0) {
      setState('idle');
      status.current?.({ state: 'idle', error: null, warnings: [], problems: [], sources: 0 });
      return;
    }
    ...
    setError(null);
    setProblems([]);
    status.current?.({ state: 'loading', error: null, warnings: [], problems: [], sources: 0 });
    let sources = 0;
    let warns: string[] = [];
    const onError = () => {
      if (cancelled || !handle) return;
      const list = [...handle.problems];
      setProblems(list);
      status.current?.({ state: 'ready', error: null, warnings: warns, problems: list, sources });
    };
    (async () => {
      ...
      const [prepared, h] = await Promise.all([prepareSpec(workspaceId, spec), createMosaic(workspaceId, { onError })]);
      handle = h;
      if (cancelled) return;
      ...
      setState('ready');
      status.current?.({ state: 'ready', error: null, warnings: warns, problems: [...handle.problems], sources });
    })().catch((e) => {
      if (cancelled) return;
      const message = (e as Error).message ?? String(e);
      setError(message);
      setState('error');
      status.current?.({ state: 'error', error: message, warnings: warns, problems: handle ? [...handle.problems] : [], sources });
    });
    return () => {
      cancelled = true;
      handle?.dispose();
  ...
  return (
    <div className={cn('mosaic-dashboard relative min-h-0', className)}>
      {state === 'loading' && <div className="absolute right-3 top-3 z-10 flex items-center gap-1.5 rounded-md border border-zinc-800 bg-zinc-900/90 px-2 py-1 text-2xs text-zinc-400"><Loader2 className="h-3.5 w-3.5 animate-spin text-accent-300" /> Rendering…</div>}
      ...
      {state === 'ready' && warnings.length > 0 && <div className="mx-3 mt-3 whitespace-pre-wrap rounded-md border border-amber-900/60 bg-amber-950/30 px-3 py-2 font-mono text-2xs text-amber-200">{warnings.map((w) => `⚠ ${w}`).join('\n')}</div>}
      {problems.length > 0 && <div className="mosaic-problems mx-3 mt-3 whitespace-pre-wrap rounded-md border border-red-900 bg-red-950/50 px-3 py-2 font-mono text-2xs text-red-200">{`${problems.length} chart quer${problems.length === 1 ? 'y' : 'ies'} failed:\n`}{problems.map((p) => `✗ ${p}`).join('\n')}</div>}
