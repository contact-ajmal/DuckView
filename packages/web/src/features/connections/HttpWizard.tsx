  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect an HTTP / REST endpoint" width="max-w-lg">
      <div className="space-y-3">
        ...
        <div><Label>Scope <span className="normal-case text-zinc-600">(optional URL prefix)</span></Label><Input value={scope} onChange={(e) => setScope(e.target.value)} className="font-mono" placeholder="https://api.example.com/" spellCheck={false} /><p className="mt-0.5 text-[11px] text-zinc-500">Only requests to URLs starting with this prefix carry the token.</p></div>
        <p className="text-[11px] text-zinc-500">Encrypted at rest, applied to your engines as a scoped <code className="font-mono">CREATE SECRET (TYPE http)</code>, never returned by the API.</p>
