            ...
            return (
              <li key={s.id} data-stream={s.name}>
                <div className="flex flex-wrap items-center gap-2 px-3 py-2">
                  ...
                  {canEdit && <Button size="sm" variant="ghost" onClick={async () => void act(`del:${s.id}`, async () => { if ((await confirmAction(`Stop and remove "${s.name}"? The table ${s.target_table} is kept.`))) { await api.del(`/api/streams/${s.id}`); await load(); } })} title="Remove (the table is kept)" aria-label="Remove (the table is kept)"><Trash2 className="h-3.5 w-3.5" /></Button>}
                </div>
