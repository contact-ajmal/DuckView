...
import { Button, Badge, Card, Input, Label, confirmAction } from '../../components/ui';
  ...
  return (
    <div className="space-y-4">
      <Card title="Google account sign-in">
        ...
        <ol className="mb-4 list-decimal space-y-1 pl-5 text-2xs text-zinc-400">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {google?.configured && <Badge tone="ok"><CheckCircle2 className="mr-1 inline h-3 w-3" /> configured{google.updated_at ? ` · ${timeAgo(google.updated_at)}` : ''}</Badge>}
          {google?.configured && <Button variant="ghost" size="sm" className="text-red-300" onClick={async () => { if ((await confirmAction('Remove the Google OAuth client? Existing Google connections stop refreshing their tokens.'))) { await api.del('/api/admin/integrations/google'); await load(); } }}><Trash2 className="h-3.5 w-3.5" /> Remove</Button>}
          {msg && <span className={msg.ok ? 'text-xs text-emerald-300' : 'text-xs text-red-300'}>{msg.text}</span>}
        </div>
        <p className="mt-3 text-2xs text-zinc-500">Server-to-server alternative: a connection can use a <b>service account key</b> instead (pasted in the connection wizard, encrypted the same way) — no OAuth client needed.</p>
      </Card>
      <SmtpCard />
    </div>
  );
  ...
  return (
    <Card title={<span className="inline-flex items-center gap-2"><Mail className="h-3.5 w-3.5" /> Outgoing mail</span>}>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {info?.configured && <Badge tone="ok"><CheckCircle2 className="mr-1 inline h-3 w-3" /> configured</Badge>}
