...
#   Python + Streamlit, pandas, pyarrow, altair, plotly and the DuckView SDK; non-root (1001), nothing else.
#   DuckView streams (Docker) or mounts (Kubernetes) the app's source, sets DUCKVIEW_URL / DUCKVIEW_TOKEN /
#   DUCKVIEW_WORKSPACE, and runs `streamlit run <entry>` with its own flags. requirements.txt, when allowed, is
...
LABEL org.opencontainers.image.title="DuckView app runtime" \
      org.opencontainers.image.description="Runs DuckView data apps (Streamlit) in their own container" \
...
RUN pip install "streamlit>=1.46" pandas pyarrow altair plotly /opt/duckview-sdk \
