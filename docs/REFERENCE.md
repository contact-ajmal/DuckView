...
The agent operates the workspace for the person who asks — from the Agent Home (`#/`, where DuckView opens; ⌘I from any page), from the Analyst WebUI (`/analyst`), from REST, or from any MCP client through the Agent MCP server. Work is organised in missions (`#/agent/missions/<id>`): an intent, chosen datasets, tasks, and the results they made. Architecture: [docs/architecture](architecture/agent-home-target-state.md) ([runtime](architecture/agent-runtime.md), [context](architecture/agent-context.md), [permissions](architecture/agent-permissions.md), [Analyst WebUI](architecture/analyst-webui.md), [Agent MCP](architecture/agent-mcp.md)). Architecture and rationale: [docs/architecture](architecture/agentic-target-state.md).
...
**Agent MCP** (`/mcp/agent`, Streamable HTTP, a token with the `mcp` scope, bound to a workspace): `ask_data_agent(request, mode?, session_id?)`, `analyse_dataset(dataset, question?)`, `investigate_data(question)`, `build_dashboard(goal)`, `create_data_app(goal)`, `explain_data(subject)`, `create_analysis(question, datasets?)`, `start_mission(request, mode?, datasets?)`, `get_mission(mission_id, wait_seconds?)`, `resume_mission(mission_id, request?)`, `get_agent_task(task_id, wait_seconds?)`, `list_agent_sessions()`. Results are a stable contract — `{taskId, sessionId, status, answer, plan, steps, artifacts, actions, approval, telemetry}` — with progress notifications while it runs. The low-level tools stay at `/mcp`. Settings → Agents creates workspace tokens and generates the client configuration.
...
| Agent | `GET /api/agent/home` · `GET /api/agent/capabilities` · `GET /api/agent/workspaces` · `GET /api/agent/workspaces/:id/datasets` · `GET/POST /api/agent/missions` · `GET/PATCH /api/agent/missions/:id` · `POST …/messages` · `…/resume` · `…/cancel` · `…/duplicate` · `GET /api/agent/artifacts[/:id]` · `POST /api/agent/artifacts/:id/run` · `GET /api/agent/config` · `GET /api/agent/tools` · `GET/POST /api/agent/sessions` · `GET/PATCH/DELETE /api/agent/sessions/:id` · `…/observations` · `POST /api/agent/tasks {workspace_id, request, session_id?, mode?, page?, wait?}` · `GET /api/agent/tasks/:id` · `GET …/events` (SSE) · `POST …/cancel` · `POST …/approval {decision}` · `GET /api/agent/approvals` · `GET/DELETE /api/agent/memory` · `GET /api/agent/telemetry` · Agent MCP: `POST|GET|DELETE /mcp/agent` |
