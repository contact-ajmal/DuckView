...
- 🤖 **Built for AI agents** — MCP server (stdio · SSE · Streamable HTTP) with 58 tools (query, profile, notebooks, comments, connections, syncs, streams, reverse ETL, Mosaic dashboards, data apps, alerts, dbt projects, metrics, automated insights, data quality, other agents over A2A), a REST façade and generated OpenAPI 3.0 for Claude Desktop, Cursor, Claude Code, Strands, LangGraph, LangChain, CrewAI, Bedrock AgentCore and Bedrock Agents. Every mutation waits for human approval.
