...
- 🤖 **Built for AI agents** — MCP server (stdio · SSE · Streamable HTTP) with 36 tools (query, profile, connections, syncs, Mosaic dashboards, data apps, alerts, dbt projects), a REST façade and generated OpenAPI 3.0 for Claude Desktop, Cursor, Claude Code, Strands, LangGraph, LangChain, CrewAI, Bedrock AgentCore and Bedrock Agents. Every mutation waits for human approval.
