...
| 🤖 **Agent-native from day one** | A hardened **MCP server** plus a **REST / OpenAPI façade** expose **49 tools**, 4 resources and 6 guided prompts to Claude Desktop, Cursor, Claude Code, Strands, LangGraph, LangChain, CrewAI, Bedrock AgentCore and Bedrock Agents. Every mutation is held for **human approval**. |
...
- **MCP server** — stdio, legacy SSE and Streamable HTTP; **49 tools**, 4 resources (workspaces, schemas, system resources, the Mosaic, data-app and dbt guides) and 6 guided prompts (`data_quality_audit`, `sql_optimization`, `build_mosaic_dashboard`, `build_data_pipeline`, `build_data_app`, `build_dbt_models`).
...
Fastify 5 (TypeScript strict)
  ...
  Agent tool registry (49 tools) → MCP + REST + OpenAPI   Copilot bridge (14 providers, write-only keys)
        │
