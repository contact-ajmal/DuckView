...
| 📓 **Notebooks** | Tell the story of an analysis in **SQL, text and inputs**: each SQL cell has a name the next one can query (`SELECT * FROM monthly`), inputs like `{{ region }}` make it re-runnable, results show as tables or charts and are saved with the notebook. Autosave with conflict protection, Markdown export, DuckView AI that writes cells for the notebook on screen, and agents that create and run notebooks over MCP. |
| 💬 **Comments & mentions** | Discuss the data where it lives — on a notebook cell, a dashboard or a table. `@mention` a teammate to bring them in (inbox + email), reply in threads, resolve when done; the bell in the top bar collects mentions and replies live. DuckView AI reads a notebook's open threads, and agents can comment too. |
...
| 🎯 **Questions answered from metrics** | Ask *"what was revenue by region?"* and, when metrics are defined, DuckView AI answers from the semantic layer: the numbers computed exactly as defined, under your access policies, with a chart. Open the answer in the Metrics explorer, add it to a dashboard, or ask the explorer directly. |
| 📈 **Automated insights** | Monitors watch your metrics day by day (or weekly, monthly) and flag values outside their usual range, weekday patterns included. They say which segments drove the change, post to Slack, Teams or email, and show it on Home. *Ask AI why* digs in. Agents use `detect_anomalies`. |
| 🧑‍💼 **Agent marketplace** | Install ready-made agents that DuckView runs for you: anomaly investigator, weekly business review, data quality auditor, pipeline watcher, catalog writer, dbt reviewer, data analyst. Or write your own. They work with read-only tools on a schedule, show every step, and post their reports to Slack, Teams or email. |
| 🤝 **Agent2Agent (A2A)** | Publish a DuckView agent and other agents can call it over A2A, with a public Agent Card and JSON-RPC with streaming. Each call runs as the caller, under their access policies. Add remote A2A agents by their card and ask them from DuckView, or let agents do it with `ask_agent`. |
| 🌊 **Streams** | Stream a **Kafka** topic (Confluent, Redpanda, MSK), an **Amazon Kinesis** stream, or events your app POSTs into a table as they arrive. New fields become columns, and offsets are committed only after rows are written. Watch the rows land, query them straight away. |
| 🔁 **Change data capture** | Mirror a **Postgres** table through logical replication: a snapshot, then every insert, update and delete, with Postgres's own types. Or mirror **Debezium** change events (MySQL, SQL Server, Oracle, MongoDB). An optional history table keeps every change. |
| 🧊 **Write back to the lakehouse** | Reverse ETL sends query results into **Apache Iceberg** tables (REST catalogs, AWS Glue, S3 Tables), with replace, append, upsert or mirror, and into **Delta Lake** tables, local or in a bucket. Other engines (Spark, Trino, Snowflake, Databricks) read them straight away. |
| 🐘 **Postgres protocol** | Tableau, Power BI, Metabase, DBeaver, Excel, psql and every Postgres driver connect straight to a workspace. Log in with your password or an API token. Access policies and token scopes apply to every query. |
| ⏱️ **Orchestration** | **Airflow** operators, **Dagster** resources and **Prefect** tasks run syncs, dbt, quality checks, reverse syncs, notebooks, agents and SQL checks. They wait for the result, and a failing check fails the pipeline. You can also call one HTTP API from any scheduler. |
...
| 🤖 **Agent-native from day one** | A hardened **MCP server** plus a **REST / OpenAPI façade** expose **61 tools**, 4 resources and 6 guided prompts to Claude Desktop, Cursor, Claude Code, Strands, LangGraph, LangChain, CrewAI, Bedrock AgentCore and Bedrock Agents. Every mutation is held for **human approval**. |
...
**The DuckView agent works in the workspace with you.** DuckView opens on the **Agent Home**: say what you are working on, choose the workspace and datasets (or let the agent find them), and pick an intent (analyse, build, investigate, automate, explore, explain) — ⌘I brings you there from any page with what you were looking at — "compare revenue by region", "why did churn go up?", "build me an executive dashboard" — and it finds the tables and the canonical metrics, runs what it needs as you (your access, your policies), shows each step in words, asks before it changes anything that matters, and leaves real objects behind in a mission workspace: charts, findings, result tables, SQL, dashboards, notebooks, checks, models, each one click from the console. Missions keep going while you work elsewhere, can be shared without widening anyone's access, and the Analyst WebUI (`/analyst`) offers the agent alone. Other agents reach it too: the **Agent MCP server** (`/mcp/agent`) takes whole tasks from Claude, Cursor or any MCP client. A provider-independent Decision Engine keeps every model call small — only the context and tools that matter. See [the architecture](docs/architecture/agentic-target-state.md).

The interface has five destinations — **Agent, Workspaces, Data, Build, Connect** — showing only what your access allows, with ⌘K for everything. Its design system, information architecture and agent UI principles are in [docs/design](docs/design/design-system.md).
...
## 🧾 Usage, cost and templates

- **Usage & cost:** Settings → Usage & cost shows what DuckView was used for and what it cost, at rates you set:
  - query and pipeline time
  - AI tokens by model
  - storage, per workspace, person and day
  - the most expensive queries

  Monthly budgets notify Slack, Teams, email, PagerDuty or a webhook when spend, or the month-end forecast, crosses a threshold.
- **Templates:** Home → Templates installs ready-made dashboards, queries, notebooks, metrics and quality checks, mapped onto your own tables or created with sample data.
  - Built in: e-commerce sales, SaaS subscriptions, web analytics and support tickets.
  - Anyone can publish a template from their own workspace; an administrator reviews it before everyone can install it.
  - Templates also travel as JSON files.
...
Fastify 5 (TypeScript strict)
  ...
  Agent tool registry (61 tools) → MCP + REST + OpenAPI   Copilot bridge (14 providers, write-only keys)
        │
